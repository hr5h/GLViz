package hr5h.glviz.shapes

object CubeShape : ShapeDefinition {

    fun newInstance(): List<Pair<FloatArray, FloatArray>> {
        val list = mutableListOf<Pair<FloatArray, FloatArray>>()

        val verticesFront = floatArrayOf(
            -0.5f, 0.5f, 0.5f,
            -0.5f, -0.5f, 0.5f,
            0.5f, -0.5f, 0.5f,
            -0.5f, 0.5f, 0.5f,
            0.5f, -0.5f, 0.5f,
            0.5f, 0.5f, 0.5f,
        )
        val colorFront = floatArrayOf(1.0f, 0.0f, 0.0f, 1.0f)
        list.add(Pair(verticesFront, colorFront))

        val verticesBack = floatArrayOf(
            -0.5f, 0.5f, -0.5f,
            0.5f, -0.5f, -0.5f,
            -0.5f, -0.5f, -0.5f,
            -0.5f, 0.5f, -0.5f,
            0.5f, 0.5f, -0.5f,
            0.5f, -0.5f, -0.5f,
        )
        val colorBack = floatArrayOf(0.0f, 1.0f, 0.0f, 1.0f)
        list.add(Pair(verticesBack, colorBack))

        val verticesLeft = floatArrayOf(
            -0.5f, 0.5f, -0.5f,
            -0.5f, -0.5f, -0.5f,
            -0.5f, -0.5f, 0.5f,
            -0.5f, 0.5f, -0.5f,
            -0.5f, -0.5f, 0.5f,
            -0.5f, 0.5f, 0.5f,
        )
        val colorLeft = floatArrayOf(0.0f, 0.0f, 1.0f, 1.0f)
        list.add(Pair(verticesLeft, colorLeft))

        val verticesRight = floatArrayOf(
            0.5f, 0.5f, 0.5f,
            0.5f, -0.5f, 0.5f,
            0.5f, -0.5f, -0.5f,
            0.5f, 0.5f, 0.5f,
            0.5f, -0.5f, -0.5f,
            0.5f, 0.5f, -0.5f,
        )
        val colorRight = floatArrayOf(1.0f, 1.0f, 0.0f, 1.0f)
        list.add(Pair(verticesRight, colorRight))

        val verticesTop = floatArrayOf(
            -0.5f, 0.5f, -0.5f,
            -0.5f, 0.5f, 0.5f,
            0.5f, 0.5f, 0.5f,
            -0.5f, 0.5f, -0.5f,
            0.5f, 0.5f, 0.5f,
            0.5f, 0.5f, -0.5f,
        )
        val colorTop = floatArrayOf(0.0f, 1.0f, 1.0f, 1.0f)
        list.add(Pair(verticesTop, colorTop))

        val verticesBottom = floatArrayOf(
            -0.5f, -0.5f, 0.5f,
            -0.5f, -0.5f, -0.5f,
            0.5f, -0.5f, -0.5f,
            -0.5f, -0.5f, 0.5f,
            0.5f, -0.5f, -0.5f,
            0.5f, -0.5f, 0.5f,
        )
        val colorBottom = floatArrayOf(1.0f, 0.0f, 1.0f, 1.0f)
        list.add(Pair(verticesBottom, colorBottom))

        return list
    }

    override fun createShapeDataList(color: FloatArray?): List<ShapeData> =
        newInstance().map { (vertices, defaultColor) ->
            ShapeData.createShapeData(vertices, color ?: defaultColor)
        }
}